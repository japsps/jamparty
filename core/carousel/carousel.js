...
exports.generateCarousel = async (search, type = "partyMap", profileId = null) => {
  try {
    ...
    addCategories(generateCategories(settings.server.modName, CloneObject(shuffleArray(allSongMapNames)), type));
    ...
    const allSongsData = SongService.getAllSongs();
    ...
    if (userProfile) {
      ...
      topArtists.forEach(artistName => {
        const artistSongs = allSongMapNames.filter(mapName => {
          const song = allSongsData[mapName];
          return song && song.artist === artistName && !playedAndFavoritedSongs.has(mapName);
        });
      ...
      if (favoriteMaps.length > 0) {
        ...
        const numBecauseYouLiked = Math.min(shuffledFavorites.length, 2);
      ...
      const recentlyPlayedMaps = (userProfile.songsPlayed || []).slice(-24).reverse();
